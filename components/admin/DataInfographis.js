...
const DataInfographis = ({ data, onSave }) => {
  ...
  const handleDeleteClick = (row) => {
    ...
    if (confirmDelete) {
      ...
      fetch(`${API_URL}infographis/deleteEvent`, {
        method: "POST",
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if(ids != null){
      body.append("id", ids);
      response = await fetch(API_URL+"infographis/editList", {
        method: "POST",
        body
      });
    } else {
      response = await fetch(API_URL+"infographis/createList", {
        method: "POST",
        body
      });
    }
...
export default DataInfographis;
