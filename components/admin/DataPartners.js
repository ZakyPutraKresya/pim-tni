...
const DataPartners = ({ data, onSave }) => {
  ...
  const handleDeleteClick = (row) => {
    ...
    if (confirmDelete) {
      ...
      fetch(`${API_URL}partners/deleteEvent`, {
        method: "POST",
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if(ids != null){
      body.append("id", ids);
      response = await fetch(API_URL+"partners/editList", {
        method: "POST",
        body
      });
    } else {
      response = await fetch(API_URL+"partners/createList", {
        method: "POST",
        body
      });
    }
