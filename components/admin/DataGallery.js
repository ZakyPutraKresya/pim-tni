...
const DataGallery = ({ data, onSave }) => {
  ...
  const handleDeleteClick = (row) => {
    ...
    if (confirmDelete) {
      ...
      fetch(`${API_URL}gallery/deleteEvent`, {
        method: "POST",
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if(ids != null){
      body.append("id", ids);
      response = await fetch(API_URL+"gallery/editList", {
        method: "POST",
        body
      });
    } else {
      response = await fetch(API_URL+"gallery/createList", {
        method: "POST",
        body
      });
    }
